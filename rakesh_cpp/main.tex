...
rakesh.kunchala@student.ncirl.ie}}
...
% ============================================================
\begin{abstract}
This paper presents the design, implementation, and deployment of TransitAccess, a serverless cloud-based accessible public transit trip planner hosted on Amazon Web Services. The application enables users to plan public transit journeys with a primary focus on accessibility requirements, providing features including transit stop management with detailed accessibility feature cataloguing, route creation with accessibility ratings, accessibility-aware route search using geographic distance calculations, favourite route management, and comprehensive accessibility statistics dashboards. The system leverages six AWS services programmatically, namely AWS Lambda for serverless compute, Amazon DynamoDB for NoSQL data storage, Amazon API Gateway for RESTful endpoint routing, Amazon S3 for static website hosting, Amazon Simple Notification Service for route update notifications, and AWS Identity and Access Management for fine-grained access control. A custom object-oriented Python library called transit-access-nci was developed and published to the Python Package Index, providing route optimisation using Haversine distance calculations, accessibility compliance checking against six defined accessibility features, transit data formatting, and input validation utilities with 47 automated tests. The application computes accessibility scores on a zero to one hundred scale for both individual stops and complete routes, enabling users with mobility, visual, or other accessibility needs to identify routes that meet their specific requirements. A CI/CD pipeline implemented through GitHub Actions automates testing, infrastructure provisioning, and deployment. The results demonstrate that serverless cloud architectures can effectively support accessibility-focused transit planning applications with geographic computation and multi-criteria search capabilities.
\end{abstract}
...
Second, the transit stop management subsystem provides complete CRUD operations for transit stops. Each stop is defined by its name, geographic coordinates (latitude and longitude), a description, and a set of boolean accessibility features drawn from the six defined categories: wheelchair\_ramp indicating the presence of ramp access to the platform, elevator indicating lift access between levels, tactile\_paving indicating textured ground surfaces for visually impaired navigation, audio\_announcements indicating audible information systems, low\_floor\_boarding indicating level boarding capability for wheelchair and mobility aid users, and accessible\_toilet indicating the availability of accessible restroom facilities. The system computes an accessibility score for each stop on a scale of zero to one hundred based on the proportion of accessibility features present.
